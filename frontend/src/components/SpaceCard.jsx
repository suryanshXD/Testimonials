...
export const SpaceCard = ({ space_id, name, description, url }) => {
  return (
    <div className="bg-zinc-800 w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl mx-auto mt-5 p-5 rounded-2xl ring-1 ring-white">
      <Link to={`/space/${space_id}`}>
        <div className="text-white text-lg sm:text-xl md:text-2xl font-semibold cursor-pointer hover:underline">
          {name}
        </div>
      </Link>
      <div className="text-neutral-200 text-xs sm:text-sm mt-2 line-clamp-3">
        {description}
      </div>
      <div className="text-neutral-300 flex flex-wrap justify-end gap-3 mt-3">
        <button onClick={() => copyUrl(url)}>
